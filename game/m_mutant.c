...
void mutant_jump_touch (edict_t *self, edict_t *other, cBspPlane_t *plane, cBspSurface_t *surf)
{
	if (self->health <= 0)
	{
		self->touch = NULL;
...
qBool mutant_check_melee (edict_t *self)
{
...
qBool mutant_check_jump (edict_t *self)
{
	vec3_t	v;
	float	distance;
...
qBool mutant_checkattack (edict_t *self)
{
