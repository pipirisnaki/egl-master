...
typedef struct m_controlsAttackMenu_s {
	uiAction_t		header;
...
} m_controlsAttackMenu_t;

typedef struct m_controlsMovementMenu_s {
	uiAction_t		header;
...
} m_controlsMovementMenu_t;

typedef struct m_controlsMiscMenu_s {
	uiAction_t		header;
...
static void KeyCursorDrawFunc (uiFrameWork_t *menu)
{
	...
	if (item->flags & UIF_LEFT_JUSTIFY)
		...
		cgi.R_DrawChar (NULL, menu->x + item->x - (((Q_ColorCharCount (item->name, (int)strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursorOffset, menu->y + item->y, UISCALE_TYPE (item->flags), UISCALE_TYPE (item->flags), 0, cursor, Q_colorWhite);
	else
		cgi.R_DrawChar (NULL, menu->x + item->x + item->cursorOffset, menu->y + item->y, UISCALE_TYPE (item->flags), UISCALE_TYPE (item->flags), 0, cursor, Q_colorWhite);
}
...
static void DrawKeyBindingFunc (void *self)
{
	...
	if (keyNums[0] == -1) {
		cgi.R_DrawString (NULL, a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*2), a->generic.y + a->generic.parent->y,
			UISCALE_TYPE (a->generic.flags), UISCALE_TYPE (a->generic.flags), 0, "???", Q_colorWhite);
		...
		cgi.R_DrawString (NULL, a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*2), a->generic.y + a->generic.parent->y,
			UISCALE_TYPE (a->generic.flags), UISCALE_TYPE (a->generic.flags), 0, name, Q_colorWhite);
		...
		if (keyNums[1] != -1) {
			cgi.R_DrawString (NULL, a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*3) + x, a->generic.y + a->generic.parent->y,
				UISCALE_TYPE (a->generic.flags), UISCALE_TYPE (a->generic.flags), 0, "or", Q_colorWhite);
			cgi.R_DrawString (NULL, a->generic.x + a->generic.parent->x + (UISIZE_TYPE (a->generic.flags)*6) + x, a->generic.y + a->generic.parent->y,
				UISCALE_TYPE (a->generic.flags), UISCALE_TYPE (a->generic.flags), 0, cgi.Key_KeynumToString (keyNums[1]), Q_colorWhite);
...
=============
*/
static struct sfx_s *ControlsMenu_Close (void)
{
...
=============
*/
static struct sfx_s *ControlsMenu_Key (uiFrameWork_t *fw, keyNum_t keyNum)
{
	char	cmd[1024];
